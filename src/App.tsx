...
import { ALL_MODIFIERS, MODIFIERS_BY_CATEGORY, MAX_SELECTED } from './data/modifiers'
    ...
    <div className="min-h-screen bg-gray-950 text-gray-100 font-sans">
        ...
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-sm font-bold text-white leading-tight truncate">Recombinant</h1>
              <p className="text-[10px] text-gray-400 leading-tight">Division 2 · Rise Up Y8S1</p>
            </div>
          </div>
          ...
          <div className="flex bg-gray-800 rounded-lg p-0.5 text-xs flex-shrink-0">
            <button
              ...
              className={`px-3 py-1.5 rounded-md font-medium transition-all ${
                activeTab === 'build' ? 'bg-gray-600 text-white' : 'text-gray-400 hover:text-white'
              }`}
            >
              🔨 Build
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-md font-medium transition-all ${
                activeTab === 'tips' ? 'bg-gray-600 text-white' : 'text-gray-400 hover:text-white'
              }`}
            >
              💡 Tips
            </button>
      ...
      <div className="max-w-2xl mx-auto px-4 pb-8">
            ...
            <section className="mt-4">
              <div className="flex items-center justify-between mb-3">
                ...
                {selected.length < MAX_SELECTED && (
                  <span className="text-[11px] text-gray-500">
                    {MAX_SELECTED - selected.length} slot{MAX_SELECTED - selected.length !== 1 ? 's' : ''} remaining
                  </span>
                )}
              </div>
              ...
              {selected.length > 0 && (
                <div className="mb-3 text-[11px] text-yellow-400/80 flex items-center gap-1.5 bg-yellow-900/10 border border-yellow-700/20 rounded-lg px-3 py-2">
                  <span>⚡</span>
                  <span>Cards marked <strong>SYNERGY</strong> pair well with your current selection.</span>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                {filteredModifiers.map(mod => (
                  <ModifierCard
                    ...
                    selectionOrder={selected.findIndex(s => s.id === mod.id) + 1 || undefined}
        ...
        <footer className="mt-8 text-center text-[11px] text-gray-600 pb-4">
          Tom Clancy's The Division 2 — Rise Up Y8S1<br />
